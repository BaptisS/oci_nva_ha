resource "null_resource" "BSFnSetup" {
  ...
  provisioner "local-exec" {
    command = "image=$(docker images | grep nva_ha | awk -F ' ' '{print $3}' | awk 'NR==1{print $1}') ; docker tag $image ${var.ocir_docker_repository}/${var.ocir_namespace}/${var.ocir_repo_name}/nva_ha:0.0.1"
    working_dir = "functions/oci_nva_ha"
  }
